...
const Careers = () => {
  ...
  const fetchCareers = async () => {
    try {
      const response = await axios.get(`${API_CONFIG.API_BASE_URL}/careers`);
  ...
  const deleteCareer = async (id) => {
    if (window.confirm("Are you sure you want to delete this application?")) {
      try {
        await axios.delete(`${API_CONFIG.API_BASE_URL}/careers/${id}`);
        fetchCareers();
      } catch (error) {
  ...
  const exportToExcel = () => {
    const exportData = careers.map((career, index) => ({
      'No.': index + 1,
      'Name': `${career.honorific} ${career.fullName}`,
      'Email': career.email,
      ...
      'Address': career.postalAddress,
      'Applied Date': new Date(career.createdAt).toLocaleDateString()
    }));

    const ws = XLSX.utils.json_to_sheet(exportData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Careers');
    XLSX.writeFile(wb, `Career_Applications_${new Date().toLocaleDateString()}.xlsx`);
  };

  if (loading) {
    return (
      <div className="loading-spinner">Loading Career Applications...</div>
    );
  }
