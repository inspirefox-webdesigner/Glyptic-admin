...
const EventsData = () => {
  ...
  const fetchRegistrations = async () => {
    try {
      const response = await axios.get(
        `${API_CONFIG.API_BASE_URL}/training/registrations`
      );
  ...
  const deleteRegistration = async (registrationId) => {
    if (window.confirm("Are you sure you want to delete this registration?")) {
      try {
        await axios.delete(
          `${API_CONFIG.API_BASE_URL}/training/registrations/${registrationId}`
        );
        fetchRegistrations();
      } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `Event_Registrations_${new Date().toLocaleDateString()}.xlsx`
    );
  };

  if (loading)
    return (
      <div className="loading-spinner">Loading Event Registrations...</div>
    );
